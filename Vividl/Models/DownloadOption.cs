...
public abstract class DownloadOption(string description, bool isAudio) : IDownloadOption
{
  public abstract string FormatSelection { get; }

  public string Description { get; } = description;

  public bool IsAudio { get; protected set; } = isAudio;
  ...
  /// </summary>
  public abstract string GetExt(string defaultValue = "");
  ...
  protected abstract Task<RunResult<string>> RunRealDownload(YoutubeDL ydl, string url,
    CancellationToken ct, IProgress<DownloadProgress> progress, OptionSet overrideOptions);

  protected abstract Task<RunResult<string[]>> RunRealPlaylistDownload(YoutubeDL ydl, string url,
    int[] playlistItems, CancellationToken ct, IProgress<DownloadProgress> progress, OptionSet overrideOptions);
    ...
    CancellationToken ct, IProgress<DownloadProgress> progress, OptionSet overrideOptions)
  {
    ...
    try
    {
      var defaultValue = video.Metadata.SelectSingleFormat(FormatSelection)?.Extension;
      if (defaultValue != null)
        ext = GetExt(defaultValue);
    }
    ...
    var fileName = Utils.Sanitize(video.DownloadName, restricted);
    var downloadPath = Path.Combine(ydl.OutputFolder, $"{fileName}.{ext}");
    // If file ext could not be resolved, we cannot make pre-download checks -> default to overwriting
    if ((Settings.Default.OverwriteMode != (Model.OverwriteMode)OverwriteMode.Overwrite) && (ext != ""))
    {
      if (File.Exists(downloadPath))
      {
        if (Settings.Default.OverwriteMode == (Model.OverwriteMode)OverwriteMode.None)
        {
          // Don't redownload if file exists.
          progress?.Report(new DownloadProgress(DownloadState.Success, data: downloadPath));
          return new RunResult<string>(true, [], downloadPath);
        }
        else
        {
  ...
  private string getNotExistingFilePath(string folder, string fileName, string ext)
  {
    ...
    while (i < 256)
    {
      var filePath = Path.Combine(folder, $"{fileName} ({i}).{ext}");
    ...
    CancellationToken ct, IProgress<DownloadProgress> progress, OptionSet overrideOptions = null)
  {
    ...
    if ((Settings.Default.OverwriteMode != (Model.OverwriteMode)OverwriteMode.Overwrite) && (ext != null))
    {
      ...
      {
        var path = Path.Combine(ydl.OutputFolder, $"{Utils.Sanitize(video.Title, restricted)}.{ext}");
...
public abstract class AudioConversionDownload(
  AudioConversionFormat format,
  string description = "") : DownloadOption(description, true)
{
  public override string FormatSelection => "bestaudio/best";

  public AudioConversionFormat ConversionFormat { get; } = format;

  public override string GetExt(string defaultValue = "")
...
public class VideoDownload(
  ...
  bool isAudio = false)
  : DownloadOption(description, isAudio)
{
  public override string FormatSelection { get; } = formatSelection;

  public VideoRecodeFormat RecodeFormat { get; } = recodeFormat;
  ...
  public override string GetExt(string defaultValue = "")
  {
    return !string.IsNullOrWhiteSpace(fileExtension) ? fileExtension : ExtProvider.GetExtForVideo(RecodeFormat, defaultValue);
  }
  ...
  protected override async Task<RunResult<string>> RunRealDownload(YoutubeDL ydl, string url,
    CancellationToken ct, IProgress<DownloadProgress> progress, OptionSet overrideOptions = null)
  {
    var formatSelection = FixFormatSelection(ydl, FormatSelection);
...
public class CustomDownload(string description, FormatData audioFormat, FormatData videoFormat)
  : DownloadOption(description, false)
{
  public FormatData AudioFormat { get; private set; } = audioFormat;
  public FormatData VideoFormat { get; private set; } = videoFormat;

  public override string FormatSelection => VideoFormat.FormatId + "+" + AudioFormat.FormatId;
