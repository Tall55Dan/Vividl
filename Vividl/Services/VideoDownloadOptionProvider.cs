...
public class VideoDownloadOptionProvider : IDownloadOptionProvider
{
  ...
  private string getFormatString(string extension)
  {
    ...
    if (App.UsingYtDlp)
    {
      s = Settings.Default.PreferRecoding ? "bv*+ba/b" : $"{extension}/bv*+ba/b";
    }
    else
    {
      s = Settings.Default.PreferRecoding ? "bestvideo+bestaudio/best" : $"{extension}/best/bestvideo+bestaudio";
    }
